...
# Connection settings (env or hardcoded)
DB_HOST = os.getenv("DB_HOST")
DB_PORT = os.getenv("DB_PORT")
DB_USER = "root"        # ← hardcoded for now (will fix later)
DB_PASSWORD = "admin"   # ← hardcoded for now (will fix later)
...
def get_engine(db_name: str = ""):
    """Cache a per-database SQLAlchemy engine. If db_name is empty, return a generic engine."""
    return create_engine(make_db_url(db_name), pool_pre_ping=True)
...
def find_allowed_dbs_for_user() -> list[str]:
    """
...
@st.cache_data(show_spinner=False)
def fetch_preview(db_name: str, table: str, limit=100, where_clause=None, order_by=None):
    """
...
def ensure_id_autocolumn(columns: list[dict]) -> list[dict]:
    ...
    """
    # find existing `id` (case-insensitive)
    id_pos = None
    ...
    if id_pos is None:
        ...
        # enforce rules on existing id (even if user tried to add it)
        c = columns[id_pos]
        c["type"] = "BIGINT"  # enforce BIGINT
        c["length"] = None
        c["precision"] = None
        c["scale"] = None
        c["nullable"] = False
    ...
    # ensure `id` is the only PK (avoid accidental composite PK)
...
def create_table(db_name: str, table_name: str, columns: list[dict]) -> tuple[bool, str]:
    ...
    # --- enforce auto `id` column as sole PK ---
    ...
    for c in columns:
        ...
        if default_raw:
            # Expect a valid SQL literal or function (e.g., 'active', 0, CURRENT_TIMESTAMP)
    ...
    # PRIMARY KEY (id) — guaranteed single-column
...
@st.cache_data(show_spinner=False)
def user_exists(username: str, host: str) -> bool:
    try:
        with get_engine("").connect() as conn:
...
def grant_privileges(username: str, host: str, db_names: list[str], mode: str) -> tuple[bool, str]:
    ...
    # Build and execute GRANT statements
    try:
        with get_engine("").connect() as conn:
            for db in db_names:
...
def main():
    ...
    with st.sidebar:
        ...
        st.caption(f"Host: {DB_HOST}:{DB_PORT}")

        allowed_dbs = find_allowed_dbs_for_user()
        if not allowed_dbs:
        ...
        st.divider()
        auto_load = st.toggle("Auto-load tables", value=True)  # default ON
        ...
        with st.expander("Create new database", expanded=False):
            ...
            with st.form("create_database_form"):
                ...
                if submitted_db:
                    ...
                    if ok:
                        st.success(msg)
                        # Refresh DB discovery and rerun
                        st.cache_data.clear()
                        st.rerun()
                    else:
                        st.error(msg)
    ...
    if DB_USER == "root":
        ...
        with st.expander("Create / Manage user", expanded=False):
            ...
            with st.form("create_user_form"):
                ...
                with grant_cols[1]:
                    # Choose DBs to grant; default to selected_db
    ...
    # --- Main: show all tables in the selected DB on one page ---
    tables = list_tables(selected_db)

    st.markdown(f"### `{selected_db}`")

    # --- Main: Create new table (builder) ---
    with st.expander("Create new table", expanded=False):
        st.caption("An `id` BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY column will be added automatically. Do not add an `id` column yourself.")
        ...
        with st.form("create_table_form"):
            ...
            # Collect per-column definitions (no primary/auto-increment controls in UI)
            columns: list[dict] = []
            for i in range(int(col_count)):
                ...
                columns.append({
                    ...
                    "nullable": nullable,
                    "unique": unique,
                    "primary": False,       # UI does not allow; id will be the sole PK
                    "auto_inc": False,      # UI does not allow; id will be AUTO_INCREMENT
            ...
            if submitted:
                ...
                if ok:
                    st.success(msg)
                    # Refresh table list & cached queries
                    st.cache_data.clear()
                    st.rerun()
                else:
                    st.error(msg)
